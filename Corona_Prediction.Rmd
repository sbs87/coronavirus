...
filter(Corona_Cases,Country.Region=="US") %>% head()
filter(Corona_Cases,Country.Region=="US_state") %>% head()

table(select(Corona_Cases,c("Country.Region","case_type")))
##------------------------------------------
## log10 transform total # cases
##------------------------------------------
...
Corona_Cases<-merge(Corona_Cases,ddply(filter(Corona_Cases,Total_confirmed_cases>100),c("case_type","Country.Region","Province.State"),summarise,case100_date=min(Date.numeric)))
Corona_Cases$Days_since_100<-Corona_Cases$Date.numeric-Corona_Cases$case100_date

# Filter df for US state-wide stats
# TODO: change to include both death and total
Corona_Cases.US_state<-filter(Corona_Cases,Country.Region=="US_state" & Total_confirmed_cases>0 & case_type=="total") 
table(select(Corona_Cases.US_state,c("Province.State","case_type")))
# Preview
head(Corona_Cases)
...
Corona_Cases.US.plotdata<-filter(Corona_Cases.US_state,Province.State %in% c("Pennsylvania","Maryland","New York","New Jersey") & City %in% c("Bucks","Baltimore City", "New York","Burlington")) 
...
(Corona_Cases.US.log.plot<-ggplot(Corona_Cases.US.plotdata,aes(x=Date,y=Total_confirmed_cases.log,col=City))+
  ...
  #facet_wrap(~case_type,scales = "free_y")+
    ggtitle(paste("Log10 cases over time,",timestamp_plot))+
...
(Corona_Cases.US.plot<-ggplot(Corona_Cases.US.plotdata,aes(x=Date,y=Total_confirmed_cases,col=City))+
  geom_point(size=4)+
    geom_line()+
  default_theme+
  facet_wrap(~Province.State,scales = "free_y",ncol=1)+
        ggtitle(paste("Cases over time,",timestamp_plot))+
theme(legend.position = "bottom",plot.title = element_text(size=12)))
...
(Corona_Cases.tristate.plot<-ggplot(filter(Corona_Cases.US.plotdata,Province.State !="New York"),aes(x=Date,y=Total_confirmed_cases,col=City))+
  geom_point(size=4)+
    geom_line()+
  default_theme+
        ggtitle(paste("MD, PA, NJ cases over time,",timestamp_plot))+
theme(legend.position = "bottom",plot.title = element_text(size=12)))
...
## US - 2 datasets (state level and country level)
