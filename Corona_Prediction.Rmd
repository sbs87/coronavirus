...
comparative longitudinal case trend|long|time|log cases|geography|none (case type?)|case_type|[15, 50, 4] geography x (2 scale?) case type    
...
#Corona_Cases.wide<-
Corona_Cases.wide<-spread(select(Corona_Cases,-c(Total_confirmed_cases.log,Population)),key =case_type ,value = Total_confirmed_cases)
Corona_Cases.wide$mortality_rate<-Corona_Cases.wide$death/Corona_Cases.wide$total
arrange(filter(Corona_Cases.wide,Country.Region=="US"),Date)
plot.dat<-filter(Corona_Cases.wide,Country.Region=="US_state" & !City %in% c("Unassigned") & mortality_rate!=Inf & mortality_rate<=1)
arrange(filter(plot.dat,mortality_rate>1),Date)
plot.dat[grepl(plot.dat$City,pattern = "New York"),]
ggplot(filter(plot.dat,Province.State %in% c("Pennsylvania","Maryland","New York","New Jersey") & City %in% c("Bucks","Baltimore City", "New York","Burlington")),aes(x=Date,y=mortality_rate,col=City))+geom_point()+
  geom_line()

# Some states have 'Unassigned' or otherwrise non cities, which when merged with other data, gets messy. Rm these instances, but keep note. 
```
