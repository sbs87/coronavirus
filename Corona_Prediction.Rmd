...
filter(Corona_Cases.US_state.tmp,grepl(x=City,pattern = "Bucks"))
tail(Corona_Cases.US_state.tmp)
unique(Corona_Cases.US_state.tmp$Province.State)
plot_data<-filter(Corona_Cases.US_state.tmp,Date.numeric==max(Corona_Cases.US_state.tmp$Date.numeric) & Province.State %in% c("Pennsylvania","Maryland","New Jersey")) %>% melt(measure.vars=c("Retail_Recreation","Workplace","Residential"))

(mobility.plot<-ggplot(plot_data,aes(y=Total_confirmed_cases,x=value))+geom_point()+
  facet_grid(Province.State~variable,scales = "free_y")+
