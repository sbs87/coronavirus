...
filter(Corona_Cases.US,Province.State %in% c("Pennsylvania")) %>% arrange(Days_since_100)
ggplot(filter(Corona_Cases.US,Province.State %in% c("Pennsylvania")),aes(x=Days_since_100,y=Total_confirmed_cases,col=City))+
  #geom_line()+
  geom_point(size=4)+default_theme+
  geom_line(data=filter(Corona_Cases.US,Province.State %in% c("Pennsylvania") & City %in% c("Bucks")) ,col='red')
