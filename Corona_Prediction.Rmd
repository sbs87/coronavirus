...
prediction_model<-function(m=1,b=0,days=1){
  ...
  prediction<-data.frame(Days_Since_x=days,Total_confirmed_cases=total_cases,Total_confirmed_cases.log=total_cases.log)
  return(prediction)
}
#today:
...
Corona_Cases$type<-"Historical"
names(Corona_Cases)
today<-max(plot_data$Days_Since_x)
tomorrow<-max(plot_data$Days_Since_x)+1
tomorrow_plus1<-tomorrow+1
tomorrow_plus2<-tomorrow+2
tomorrow_plus3<-tomorrow+3
...
Corona_Cases_wprediction<-rbind.fill(Corona_Cases,data.frame(Code="USA",type="Prediction",prediction_model(m=slope,b=intercept,days = predicted_days)))
plot_data.prediction<-filter(Corona_Cases_wprediction,Code=="USA" & Days_Since_x>0)



(predicted.plot<-ggplot(plot_data.prediction,aes(x=Days_Since_x,y=Total_confirmed_cases.log,col=type))+
    geom_abline(slope = slope,intercept = intercept,lty=2)+
...
default_theme+
  ...
  theme(legend.position = "bottom")+
    geom_text(data = data.frame(type="Prediction",Days_Since_x=predicted_days-7,Total_confirmed_cases.log=log(c(56490.42,75400.32,100640.22,319422.18),10),text_display=paste0(c("tomorrow : ","two_days: ","three_days: ","next_week: "),round(c(56490.42,75400.32,100640.22,319422.18),digits = 0))),aes(label=text_display))+
  scale_color_manual(values = c("Historical"="#377eb8","Prediction"="#e41a1c")))
  
write_plot(predicted.plot,wd = results_dir)
