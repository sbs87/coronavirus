...
ggplot(filter(Corona_Cases.US,Province.State %in% c("Pennsylvania")),aes(x=Days_since_100,y=Total_confirmed_cases,col=City))+geom_line()+geom_point(size=2)+default_theme
...
(historical_model_predictions.plot<-ggplot(rbind.fill(historical_model_predictions,data.frame(Corona_Cases.US,type="Historical")),aes(x=Days_since_100,y=Total_confirmed_cases.log,col=type))+
    geom_point(size=3)+
    default_theme+
